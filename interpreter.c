...
int mount(char* words[]){
    int blocks = (int) words[2];
    int block_size = (int) words[3];
    ...
    if (fp){
        ...
        int error = partition(words[1], blocks, block_size);
        if (error == 0){
            return mountFS(words[1]);
        }else{
            return error;
...
int write (char* words[]){
    if ( (words[2][0] != '[') || (words[words[countwords(words)-1][strlen(countwords(words)-1)-1]] !=']')){
...
int interpreter(char* words[]){
    ...
    if ( strcmp(words[0],"help") == 0 ) {
        ...
        if ( strcmp(words[1],"_NONE_") == 0  || strcmp(words[2],"_NONE_") == 0 || strcmp(words[3],"_NONE_") == 0|| strcmp(words[4],"_NONE_") != 0 ) return -2;
        errorCode = mount(words);
    } else if (strcmp(words[0],"write") == 0){
        if ( strcmp(words[2],"_NONE_") == 0 ) return -2;
